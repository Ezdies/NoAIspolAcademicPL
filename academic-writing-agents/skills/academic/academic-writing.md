# Academic Writing Principles for Polish Scientific Work

Canonical reference for `academic-writing-agents` in `/home/ezdies/AntiSlopPL`.
Before applying these principles, read `/home/ezdies/AntiSlopPL/POLISH_ACADEMIC_CORE.md`.
If this file conflicts with `POLISH_ACADEMIC_CORE.md`, the core file wins.

These principles are organized into 6 categories that map to agent specializations.
...
For Polish prose, Category B means: preserve author voice, remove GenAI filler, remove English calques, keep warranted hedging, and prefer natural Polish academic syntax over literal English-style rewrites.

### B1. Avoid Exhaustive-Sounding Enumerations
When listing examples, do not imply completeness unless the list is complete. In Polish, prefer "takich jak" for illustrative lists and explicit labels for complete sets.
...
Avoid formulaic contrast structures when they only create rhetorical balance. In Polish, overused patterns include "nie tylko X, lecz także Y", "z jednej strony... z drugiej strony", and "nie chodzi o X, ale o Y". Keep them only when the contrast is necessary.
...
Discussion/findings should flow as analytical prose, not flat report-style enumeration. Match the actual author's Polish voice when a sample exists. Do not force first-person plural or active voice; Polish academic prose often uses impersonal forms such as "przeanalizowano" and "wykazano" when appropriate.
...
Every word must earn its place. In Polish, cut filler phrases such as "warto zauważyć", "należy podkreślić", "w ramach niniejszej pracy", "istotnym aspektem jest", and "w kontekście dynamicznego rozwoju". Prefer clear Polish verbs and nouns over nominalized chains.

**Common violations**: "warto zauważyć" (cut entirely); "dokonanie analizy" -> "analiza" or "przeanalizowano"; "adresować problem" -> "podejmować/rozwiązywać problem"; "odgrywa kluczową rolę" without mechanism; restating in the conclusion what was already said with only cosmetic changes.

### B8. AI-Writing Tell Detection
Actively scan for Polish GenAI markers from `POLISH_ACADEMIC_CORE.md`: empty metadiscourse, inflated significance, mechanical transitions, English calques, abstract noun subjects, nominalizations, false completeness, and uniform paragraph rhythm. Replace them with direct, specific Polish language grounded in the supplied material.

**Common violations**: Three consecutive paragraphs starting with "ponadto" / "co więcej" / "jednocześnie"; using "adresować problem" instead of "podejmować" or "rozwiązywać problem"; "krajobraz badawczy" where "stan badań" or "literatura" is clearer; sentences that begin by restating the previous sentence rather than advancing the argument.
...
### F2. Formulaic Contrast Audit
Before finalizing any chapter, search for formulaic contrast patterns and rephrase them when they only create rhetorical symmetry. In Polish, check "nie tylko... ale także", "z jednej strony... z drugiej strony", "nie X, lecz Y", and "nie chodzi o X, ale o Y".

**Common violations**: "Celem nie jest X, lecz Y" when the text can simply state "Celem jest Y"; "nie tylko... ale także" used to combine two claims that should be separate sentences; balanced contrast used without a real conceptual contrast.
