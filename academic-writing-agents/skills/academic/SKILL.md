...
name: academic
description: >-
  Academic writing multi-agent orchestrator with Polish academic writing support.
  TRIGGER when: user is editing .tex files, reviewing thesis/paper chapters,
  drafting academic content in Polish or English, checking writing quality,
  removing GenAI-slop, or analyzing research positioning. Coordinates
...
In `/home/ezdies/AntiSlopPL`, default to Polish academic writing unless the user asks otherwise. The goal is not generic "humanization"; it is evidence-bounded Polish prose with author voice and without GenAI filler, inflated claims, English calques, or false causal certainty.
...
1. Read `/home/ezdies/AntiSlopPL/POLISH_ACADEMIC_CORE.md` for shared Polish academic and anti-GenAI rules.
2. Read `academic-writing.md` (in the same directory as this skill) for writing principles organized in 6 categories (A. Structure & Narrative, B. Prose & Style, C. Math & Equations, D. Figures & Tables, E. Citations & Bibliography, F. Process & Meta).
3. If a project-level `AGENTS.md`, `START_HERE_PL.md`, or `.claude/CLAUDE.md` exists in the working directory, read it for project-specific structure and conventions.
4. Check for project-level agents: Glob for `.claude/agents/*.md` in the working directory. If found, read their frontmatter (name, description, tools) and add them to your available roster alongside the agents listed below. Present project agents in your deployment plan.
5. Include relevant context (principles, project info, workflow triggers, Polish academic constraints) in each agent's deployment prompt. Reference principle categories relevant to deployed agents.

## Available Worker Agents

Use their name as `subagent_type` when spawning via the Agent tool. If the Agent/subagent tool is not available in the current Codex session, read the relevant files in `/home/ezdies/AntiSlopPL/academic-writing-agents/agents/` and apply them as reviewer roles/checklists yourself, then synthesize one coherent report.
...
For Polish academic work, every deployment prompt must include: preserve claims and uncertainty, do not invent citations or results, flag missing evidence as `Do weryfikacji`, and scan for Polish GenAI markers listed in `POLISH_ACADEMIC_CORE.md`.

### Step 2: Deploy Workers

Rules for deployment:
- **Maximize parallelism**: Launch all independent agents simultaneously in a single response.
- **Codex fallback**: If dedicated worker agents are unavailable, do not block. Apply the relevant agent instructions directly as staged review passes.
...
When synthesizing Polish writing feedback, merge duplicate "AI tell" findings into a single `Ślady GenAI` group so the user does not receive repeated style complaints from multiple agents.
...
1. **Jaka jest główna teza?** — Jaką jedną myśl czytelnik ma zapamiętać? (A7)
2. **Kto jest odbiorcą?** — Promotor, komisja, recenzenci konferencji, czytelnicy interdyscyplinarni?
3. **Co jest przed i po tej części?** — Kontekst przejść logicznych (A2)
4. **Jakie figury/tabele istnieją?** — Żeby tekst mógł je interpretować, nie tylko wskazywać (D2, D7)
5. **Jakie prace muszą być cytowane?** — Podstawowe pozycjonowanie i źródła twierdzeń (E1, E2)
...
4. **Final checklist**: Compile a submission readiness report covering:
   ...
   - [ ] Formulaic contrast audit passed (F2)
...
```markdown
## Synteza orkiestratora

### Ocena ogólna
[1-2 zdania podsumowania]

### Krytyczne problemy (N)
...
### Ślady GenAI
- [frazy, kalki, rytm, inflacja znaczenia]
...
### Do weryfikacji
- [brakujące dane, cytowania, decyzje autora]
